...
- **ClipStore**: sharded by `clip_id` (see [sharding](../foundations/sharding.md) (id: sharding)). Point lookup only.
...
- HotPath stampede on a viral clip → local LRU on the Resolve API plus single-flight fill. See [caching](../foundations/caching.md) (id: caching).
- Key exhaustion in one DC's range → steal from MintPool global, do not wrap into used space.

## Follow-ups an interviewer may ask

- Custom aliases: extra unique index, abuse, and a different rate limit. See [rate-limiter](./rate-limiter.md) (id: rate-limiter).
