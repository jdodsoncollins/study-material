...
Shard only if the snapshot no longer fits in one box: shard by **first two characters** (`ha*`, `hb*`, …). The API knows the map. See [sharding](../foundations/sharding.md) (id: sharding) and [indexes](../../cs/indexes.md) (id: indexes).
...
[QuotaDesk](./rate-limiter.md) (id: rate-limiter) per IP so a scraper cannot walk the trie.
